...
p <- VlnPlot(test2, ncol = 3, features = genes)

# install.packages("ggrastr")  # if not already installed
library(ggrastr)
library(Seurat)

# Rebuild the VlnPlot but rasterize the point layer
p <- VlnPlot(tec, features = my_genes, pt.size = 0.1) 
p_rast <- rasterise(p, layers = "Point", dpi = 300)

ggsave("vlnplot_rasterized.pdf", p_rast, width = 8, height = 30)

# install.packages("magick")
library(magick)
library(pdftools)

img <- image_read_pdf("vlnplot_rasterized.pdf", density = 300)   # rasterize at 300 dpi
image_write(img, "vln_rasterized2.pdf", format = "pdf")
# or export as PNG:
image_write(img, "vln_rasterized.png", format = "png")
