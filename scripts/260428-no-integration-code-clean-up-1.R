...
# LOAD DATA, INITIATE SEURAT OBJECTS
...
FeaturePlot_scCustom(samples, reduction = "umap", features = "UBD", colors_use = viridis_light_high, label = TRUE, label.size = label_size, order = order)  # need to double check
# All TECs
FeaturePlot_scCustom(samples, reduction = "umap", features = "TP63", colors_use = viridis_light_high, label = TRUE, label.size = label_size, order = order)  # need to double check
...
saveRDS(test, file = "data/rds-objects/260428-after-annotation.rds")
test <- readRDS("data/rds-objects/260428-after-annotation.rds")
