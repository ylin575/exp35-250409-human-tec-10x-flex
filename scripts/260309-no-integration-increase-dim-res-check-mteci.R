...
DimPlot(samples, 
        reduction = "umap",
        label = TRUE, 
        cells.highlight = WhichCells(samples, idents = 26))
DimPlot(samples, 
        reduction = "umap",
        label = TRUE, 
        cells.highlight = WhichCells(samples, idents = 8),
        split.by = "sample.id")
DimPlot(
  samples,
  reduction = "umap",
  cells.highlight = WhichCells(samples, idents = 5),
  cols = "grey85",
  cols.highlight = "red",
  sizes.highlight = 0.5
)
...
test3 <- samples[, samples@meta.data$seurat_clusters==8]
DimPlot(test3, reduction = "umap", label = TRUE, split.by = "sample.id")

plot1 <- FeatureScatter(test3, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(test3, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
plot1 <- FeatureScatter(samples, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(samples, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
plot1 <- FeatureScatter(samples, feature1 = "nCount_RNA", feature2 = "percent.mt", split.by = "seurat_clusters")
plot1
...
FeaturePlot(samples, reduction = "umap", features = "RYR3", label = TRUE, label.size = label_size, order = order)
...
FeaturePlot(samples, reduction = "umap", features = "ITGB4", label = TRUE, label.size = label_size, order = order)
# CCL21-positive TEC
...
# Endothelial
FeaturePlot(samples, reduction = "umap", features = "PECAM1", label = TRUE, label.size = label_size, order = order)
FeaturePlot(samples, reduction = "umap", features = "BCAM", label = TRUE, label.size = label_size, order = order)
...
FeaturePlot(samples, reduction = "umap", features = "CLEC2L", order = order, split.by = "sample.id")
...
DimPlot(test, reduction = "umap", label = FALSE, pt.size = 0.5, split.by = "sample.id")
DimPlot(test, reduction = "umap", label = TRUE, pt.size = 0.5) #+ NoLegend()
test@meta.data$cell_type <- Idents(test)
genes <- c("CCL19","AIRE","FEZF2","LY75","PRSS16","PSMB11","COL4A5","COL4A6",
           "ITGA6","BCAM","EPCAM","CD24","ITGB4")
FeaturePlot_scCustom(test, 
                     reduction = "umap", 
                     features = "CD74",
                     label = FALSE,
                     ...
                     na_color = "lightgray",
                     combine = FALSE)
genes <- c("CCL19","AIRE","FEZF2","LY75","PRSS16","PSMB11")
FeaturePlot_scCustom(test, 
                     reduction = "umap", 
                     features = "PSMB11",
                     label = FALSE,
                     ...
                     combine = FALSE,
                     split.by = "sample.id")
FeaturePlot(test, reduction = "umap", features = "CCL19", label = TRUE, label.size = label_size, order = order)  # need to double check
FeaturePlot(test, reduction = "umap", features = "AIRE", label = TRUE, label.size = label_size, order = order, split.by = "sample.id")  # need to double check
FeaturePlot(test, reduction = "umap", features = "LY75", label = TRUE, label.size = label_size, order = order, split.by = "sample.id")  # need to double check
FeaturePlot(test, reduction = "umap", features = "PRSS16", label = TRUE, label.size = label_size, order = order, split.by = "sample.id")  # need to double check
FeaturePlot(test, reduction = "umap", features = "PSMB11", label = TRUE, label.size = label_size, order = order, split.by = "sample.id")  # need to double check
DimPlot(test, reduction = "umap", label = TRUE, group.by = "cell_type", split.by = "sample.id")
...
# test2 <- test[, test@meta.data$cell_type=="mTEC I"]
# DimPlot(test2, reduction = "umap", label = TRUE, group.by = "cell_type", split.by = "sample.id")

# save object after annotation
saveRDS(test, file = "data/rds-objects/260309-after-annotation.rds")
test <- readRDS("data/rds-objects/260309-after-annotation.rds")
...
cluster8.markers <- FindMarkers(samples, ident.1 = 8, logfc.threshold = 0.25,
